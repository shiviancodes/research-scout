...
2. If `energy` is `false` (or the file is missing), proceed directly to `## Sources` and run a full web research pass.
3. If `energy` is `true`, this is an **upload-first run**. Uploaded content is your primary research material. Follow steps 3a–3h before doing any web research.
   ...
   e. **Immediately** update `inputs/settings.json` by setting `energy` to `false`.
